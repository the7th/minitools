...
<section class="rounded-3xl border border-plum/10 bg-tea/60 p-8 shadow-xl shadow-plum/5">
    <p class="text-xs font-semibold uppercase tracking-[0.3em] text-plum/60">Compress PDF</p>
    <h1 class="mt-3 text-3xl font-bold tracking-tight"><?= t('pdf.heading') ?></h1>
    <p class="mt-3 text-sm leading-relaxed text-plum/70">
        <?= t('pdf.intro', ['focus' => '<span class="font-semibold text-plum">' . t('pdf.focus') . '</span>']) ?>
    </p>
    ...
    <form action="/tools/compress-pdf" method="post" enctype="multipart/form-data" class="mt-8 space-y-4">
        <label for="pdf" class="block text-sm font-medium text-plum/80"><?= t('pdf.label') ?></label>
        <input
            id="pdf"
            name="pdf"
            type="file"
