        ...
        <nav class="mb-4 flex items-center justify-end gap-1 text-xs" aria-label="<?= e(t('lang.label')) ?>">
            <?php foreach (lang_choices() as $locale => $label): ?>
                ...
                <a
                    ...
                    class="<?= $active ? 'rounded-lg bg-slate-800 px-3 py-1.5 font-semibold text-slate-100' : 'rounded-lg px-3 py-1.5 font-semibold text-slate-500 transition hover:text-slate-300' ?>"
                ><?= e($label) ?></a>
            <?php endforeach; ?>
        </nav>
