...
<div class="space-y-6">
    <section class="rounded-3xl border border-slate-800 bg-slate-900/60 p-8 shadow-2xl shadow-black/40">
        <p class="text-xs font-semibold uppercase tracking-[0.3em] text-emerald-400"><?= t('project.eyebrow') ?></p>
        <h1 class="mt-3 text-3xl font-bold tracking-tight"><?= $p['title'] ?></h1>
        <p class="mt-4 text-base leading-relaxed text-slate-300"><?= $p['tagline'] ?></p>

        <dl class="mt-6 grid gap-3 sm:grid-cols-3">
            <div class="rounded-xl border border-slate-800 bg-slate-950 px-4 py-3">
                <dt class="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500"><?= t('project.meta_role') ?></dt>
                <dd class="mt-1 text-sm font-semibold text-slate-200"><?= $p['role'] ?></dd>
            </div>
            <div class="rounded-xl border border-slate-800 bg-slate-950 px-4 py-3">
                <dt class="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500"><?= t('project.meta_period') ?></dt>
                <dd class="mt-1 text-sm font-semibold text-slate-200"><?= $p['period'] ?></dd>
            </div>
            <div class="rounded-xl border border-slate-800 bg-slate-950 px-4 py-3">
                <dt class="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500"><?= t('project.meta_type') ?></dt>
                <dd class="mt-1 text-sm font-semibold text-slate-200"><?= $p['type'] ?></dd>
            </div>
        </dl>
    </section>
    ...
    <section class="rounded-3xl border border-slate-800 bg-slate-900/60 p-8 shadow-2xl shadow-black/40">
        ...
        <p class="mt-4 text-base leading-relaxed text-slate-400"><?= $p['context'] ?></p>
    </section>
    ...
    <section class="rounded-3xl border border-slate-800 bg-slate-900/60 p-8 shadow-2xl shadow-black/40">
        ...
        <ul class="mt-5 flex flex-wrap gap-2">
            <?php foreach ($p['tech'] as $tech): ?>
                <li class="rounded-lg border border-slate-700 bg-slate-950 px-3 py-1.5 text-xs font-semibold text-slate-300"><?= $tech ?></li>
            <?php endforeach; ?>
        </ul>
    </section>
</div>
